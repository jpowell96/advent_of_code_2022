...
fs.readFile("input.txt", "utf-8", function read(err, data) {
    ...
    var totalDirectorySize = fileSystemTree.size;
    var totalDiskSpace = 70000000;
    var totalUnusedSpace = totalDiskSpace - totalDirectorySize;
    // Find x, such that totalDiskSpace - (totalDirectorySize - x) > 30000000
    function willAllowDiskSpaceDownload(treeNode) {
        return totalDiskSpace - (totalDirectorySize - treeNode.size) > 30000000;
    }
    // 3. Find the directories less than 100k
    var result = findAll(willAllowDiskSpaceDownload, fileSystemTree);
    var asSizes = result.map(function (node) { return (node === null || node === void 0 ? void 0 : node.size) ? node.size : Number.MAX_VALUE; });
    console.log(Math.min.apply(Math, asSizes));
});
